...
#define CHUNKSIZE 10
#define IMPL 0
static const char FILEPATH[] = "../data/breastcancer1.csv";
...
int main (int argc, char *argv[])
{
    ...
    double end;
    struct data dataset = loadDataset(FILEPATH, "\t");
...
void copyMatrix(float **mat1, float **mat2, int row, int col){ //mat1 dest, mat2 source
  // I/O operazioni non richiedono molto lavoro da parte della CPU, ma piuttosto dalla memoria
  for (int i = 0; i < row; i++) {
    for (int j = 0; j < col; j++) {
        mat1[i][j] =  mat2[i][j];
    }
...
void writeFile(float** data ,int **clusters, int n, int m) {
	FILE* fd;
	fd = fopen("out/output_breastcancer_1.txt", "w");
	for(int i = 0; i < n; i++) {
		fprintf(fd, "%d", i);
          for(int j = 0; j < k; j++) {
